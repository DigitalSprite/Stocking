...
def _parsing_dayprice_json(types=None, page=1, flag=0):
    """
           处理当日行情分页数据，格式为json
     Parameters
     ------
    ...
    """
    # ct._write_console()
    myurl = ct.SINA_DAY_PRICE_URL % (ct.P_TYPE['http'], ct.DOMAINS['vsf'],
...
def get_today_all(flag=0,start=1,end = 60,st = False):
    """
    ...
    try:
        # print 'get'
        df = _parsing_dayprice_json('hs_a', start, flag)
    except Exception,e:
        print "the first None"
        df = None
    ...
    if df is not None:
        ...
        for i in range(start + 1, end):
            try:
                newdf = _parsing_dayprice_json('hs_a', i, flag)
            except Exception,e:
                # print e
                # print "get data error"
                continue
    ...
    # if st:
    #     try:
    #         df = df.append(_parsing_dayprice_json('shfxjs', 1, flag),
    #                                                ignore_index=True)
    #     except Exception,e:
    #         # print "get data error"
    #         # print e
